...
export function nativeInvoice(invoice: Invoice, options?: Pick<RenderOptions, "fonts">) {
  const net = invoice.items.reduce((total, item) => total + item.quantity * item.unitPrice, 0);
  const tax = net * invoice.taxRate;
    ...
    >
        ...
        {invoice.items.map((item) => (
          <Line
            ...
            cells={[
              ...
              money(invoice.currency, item.quantity * item.unitPrice),
            ]}
          />
        ))}
      </div>
      ...
      >
        ...
        <Line
          ...
          color={muted}
        />
        <Line cells={["", "", "Total due", money(invoice.currency, net + tax)]} bold />
      </div>
    </main>,
